...
import { db } from "../(site)/firebase/firebaseConfig"; // Import the Firestore configuration from the firebaseConfig file.
			...
			entries.map(
				(entry) =>
					entry.id === id
						...
						: entry // Toggle isEditing flag for the selected entry
			)
		);
	};
						...
						<div key={entry.id} className="entry-item">
							...
							{entry.isEditing ? ( // Check if the entry is in editing mode
								<textarea
									...
									} // Update the entry content on change
								/>
							) : (
